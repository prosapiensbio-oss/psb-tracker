...
import { opakovaneZrusenia } from "./compute";

describe("opakovaneZrusenia — dve zrušenia za 30 dní sú signál", () => {
  const dnes = new Date("2026-08-21T12:00:00Z");
  it("nájde klienta s dvoma zrušeniami v okne", () => {
    const z = [
      { druh: "zrusene", klient: "Anna", pred: "2026-08-05T17:00" },
      { druh: "zrusene", klient: "Anna", pred: "2026-08-19T17:00" },
    ];
    expect(opakovaneZrusenia(z, dnes)).toEqual([{ klient: "Anna", dni: ["2026-08-05", "2026-08-19"] }]);
  });
  it("jedno zrušenie nie je vzor", () => {
    expect(opakovaneZrusenia([{ druh: "zrusene", klient: "Anna", pred: "2026-08-19T17:00" }], dnes)).toEqual([]);
  });
  it("staré zrušenie mimo okna sa neráta", () => {
    const z = [
      { druh: "zrusene", klient: "Anna", pred: "2026-07-01T17:00" },
      { druh: "zrusene", klient: "Anna", pred: "2026-08-19T17:00" },
    ];
    expect(opakovaneZrusenia(z, dnes)).toEqual([]);
  });
  it("ručný zápis a zistenie z kalendára o tom istom dni sú jedno zrušenie", () => {
    const z = [
      { druh: "zrusene", klient: "Anna", pred: "2026-08-19T17:00" },
      { druh: "zrusene", klient: "Anna", pred: "2026-08-19T17:00" },
    ];
  ...
  it("posun ani pridanie sa nerátajú, zrušený budúci termín áno", () => {
    const z = [
      { druh: "posunute", klient: "Anna", pred: "2026-08-10T17:00", po: "2026-08-11T17:00" },
      { druh: "zrusene", klient: "Anna", pred: "2026-08-12T17:00" },
      { druh: "zrusene", klient: "Anna", pred: "2026-08-26T17:00" },
    ];
    expect(opakovaneZrusenia(z, dnes)).toEqual([{ klient: "Anna", dni: ["2026-08-12", "2026-08-26"] }]);
  });
});
